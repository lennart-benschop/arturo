...
//		Reviewed :	No
//		Purpose :	File I/O mapping (heavily based on fileio.c in hardware)
//
// ***************************************************************************************
// ***************************************************************************************
...
struct _FIO_Object {
	...
	int fatfsError;  																// Recorded fatFS error, debugging purposes only.
...
static int _FIO_MapError(int e) {
	int err;
...
static void _FIOError(int h,bool isError) {
	file[h].error = FIO_OK;  														// Set to ok, save fatfs result 
	file[h].fatfsError = errno; 
	if (isError) {  																// Error occurred, remap
		file[h].error = _FIO_MapError(errno);
	}
}
